...
void global_mission_spec(float proba_vect[PROBA_VECT_SIZE], float reward_vect[REWARD_VECT_SIZE],
...
void Mdp_Global(float proba_vect[PROBA_VECT_SIZE], float reward_vect[REWARD_VECT_SIZE], float discount, float policy[MATRIX_SIZE][1], float V[MATRIX_SIZE][1]){
	float P[MATRIX_SIZE][MATRIX_SIZE][NB_ACTION], R[MATRIX_SIZE][NB_ACTION];

	global_mission_spec(proba_vect, reward_vect, P, R);
	Mdp_Policy_Iteration_PI(P, R, discount, policy, V);
}
